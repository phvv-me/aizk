<script lang="ts">
  ...
  const memoryUsedPct = $derived(
    data.hardware
      ? percent(
          data.hardware.memory_total_bytes !== null && data.hardware.memory_available_bytes !== null
            ? data.hardware.memory_total_bytes - data.hardware.memory_available_bytes
            : null,
          data.hardware.memory_total_bytes
        )
      : null
  );
  const diskUsedPct = $derived(
    data.hardware
      ? percent(
          data.hardware.disk_total_bytes !== null && data.hardware.disk_available_bytes !== null
            ? data.hardware.disk_total_bytes - data.hardware.disk_available_bytes
            : null,
          data.hardware.disk_total_bytes
        )
      : null
  );
</script>
...
{#if !data.health}
  <Card.Root class="mb-8">
    <Card.Header>
      <Card.Title>Health is unavailable</Card.Title>
...
{:else}
  <div class="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
    <Card.Root class="gap-2 py-4">
      <Card.Header class="px-4">
        <Card.Description>Migration</Card.Description>
        <Card.Title class="flex items-center gap-2 text-lg">
          <Badge variant={data.health.migration.up_to_date ? 'secondary' : 'destructive'}>
            {data.health.migration.up_to_date ? 'At head' : 'Behind head'}
          </Badge>
        </Card.Title>
        <p class="text-muted-foreground mt-1 text-xs">
          {data.health.migration.current ?? 'unstamped'} &rarr; {data.health.migration.head}
        </p>
      </Card.Header>
    </Card.Root>
    <Card.Root class="gap-2 py-4">
      <Card.Header class="px-4">
        <Card.Description>RLS violations</Card.Description>
        <Card.Title class="text-3xl">{data.health.rls_violations.length}</Card.Title>
      </Card.Header>
    </Card.Root>
    <Card.Root class="gap-2 py-4">
      <Card.Header class="px-4">
        <div class="flex items-center gap-2">
          <Card.Description>Queue</Card.Description>
          <InfoTip
            label="What queue counts mean"
            text="Pending waits to run, running holds a live worker lease, and failed is retained for repair. See Queues for detail by entrypoint."
          />
        </div>
        <Card.Title class="text-lg"
          >{data.health.queue.pending} pending &middot; {data.health.queue.running} running</Card.Title
        >
        <p class="text-muted-foreground mt-1 text-xs">
          {data.health.queue.failed} failed &middot;
    ...
    <Card.Root class="gap-2 py-4">
      <Card.Header class="px-4">
        ...
        <Card.Title class="text-lg">{formatBytes(data.health.storage.stored_bytes)}</Card.Title>
        <p class="text-muted-foreground mt-1 text-xs">
          {formatBytes(data.health.storage.compression_saved_bytes)} saved by compression &middot;
  ...
  {#if data.health.rls_violations.length > 0}
    <Card.Root class="border-destructive mb-8">
      ...
      <Card.Content>
        <ul class="list-inside list-disc space-y-1 text-sm">
          {#each data.health.rls_violations as violation (violation)}
            <li>{violation}</li>
          {/each}
        </ul>
      </Card.Content>
  ...
  <div class="mb-8 grid gap-6 xl:grid-cols-2">
    <Card.Root>
      <Card.Header>
        ...
        <Card.Description>Reachability and served identity for each model lane.</Card.Description>
      </Card.Header>
      <Card.Content>
        {#if data.health.endpoints.length === 0}
          <p class="text-muted-foreground text-sm">No serving endpoints are configured.</p>
        {:else}
          <ul class="divide-border divide-y">
            {#each data.health.endpoints as endpoint (endpoint.name)}
    ...
    <Card.Root>
      ...
      <Card.Content>
        <dl class="grid grid-cols-2 gap-x-4 gap-y-2 text-sm sm:grid-cols-3">
          {#each Object.entries(data.health.row_counts) as [table, count] (table)}
            <div>
  ...
  <Card.Root class="mb-8">
    ...
    <Card.Content>
      ...
      {:else}
        <div class="mb-5 grid gap-5 sm:grid-cols-3">
          <div>
            ...
            <dd class="mt-1 text-lg">
              {data.hardware.load1?.toFixed(2) ?? 'N/A'} / {data.hardware.load5?.toFixed(2) ??
                'N/A'} / {data.hardware.load15?.toFixed(2) ?? 'N/A'}
            </dd>
          </div>
          <div>
            ...
            <dd class="mt-1 text-lg">
              ...
              {#if data.hardware.memory_total_bytes !== null}
                <span class="text-muted-foreground text-xs"
                  >of {formatBytes(data.hardware.memory_total_bytes)}</span
                >
              {/if}
            </dd>
          </div>
          <div>
            ...
            <dd class="mt-1 text-lg">
              ...
              {#if data.hardware.disk_total_bytes !== null}
                <span class="text-muted-foreground text-xs"
                  >of {formatBytes(data.hardware.disk_total_bytes)}</span
                >
              {/if}
            </dd>
          </div>
        ...
        <ul class="divide-border divide-y">
          {#each data.hardware.lanes as lane (lane.service)}
            <li class="flex flex-wrap items-center justify-between gap-2 py-2.5 first:pt-0">
  ...
  <Card.Root>
    <Card.Header>
      ...
      <Card.Description>Confirm end-to-end retrieval against real stored memory.</Card.Description>
      <Card.Action>
        <form
          method="POST"
          action="?/recall"
