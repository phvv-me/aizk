...
async def curated_groups_administered(user_id: uuid.UUID) -> list[Group]:
    """The curated groups (groups whose writes need admin review before they count as approved
    canon) this user holds the admin membership role in.

    `Membership` and `Group` carry no row level security of their own, so this reads the same
    under this user's own session as it would under any other. The review pass's own
...
async def debounced(user_id: uuid.UUID, group: Group, pending_count: int) -> bool:
    ...
    user_id: the admin member this pass reviews on behalf of, who owns the watermark, the
...
async def apply_verdicts(
    ...
    system user's elevated reach. `curated_groups_administered` already vetted this pass's own
    user as the group's own admin member before this ever runs.
...
async def review_group(user_id: uuid.UUID, group: Group) -> tuple[int, int]:
    ...
    The watermark itself is this user's own private bookkeeping row, so it is read and
    written under this user's own session, the scope its write policy's WITH CHECK admits.
...
async def review_curated_groups(user_id: uuid.UUID) -> int:
    """Review every curated group this user administers, return the total claims judged.

    user_id: the admin member whose curated groups are reviewed.
    """
    groups = await curated_groups_administered(user_id)
    judged = 0
    for group in groups:
        approved, rejected = await review_group(user_id, group)
