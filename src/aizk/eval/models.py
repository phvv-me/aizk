...
class QA(FrozenModel):
    """One retrieval probe paired with its expected fact when known."""
...
class EvalReport(FrozenModel):
    """Aggregate retrieval quality measured on memory already stored in Aizk."""
    ...
    def render(self) -> str:
        """One scorecard line plus the per-config split the sweep measured."""
        judged = f" judge={self.mean_judge:.3f}" if self.mean_judge is not None else ""
        configs = "\n".join(f"{label}: {score:.3f}" for label, score in self.per_config.items())
        summary = (
            f"n={self.n} hit@k={self.hit_at_k:.3f} "
            f"ndcg@k={self.ndcg_at_k:.3f} mrr={self.mrr:.3f}{judged}"
        )
