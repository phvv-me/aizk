...
class EntityGate(Singleton):
    ...
    Scores a chunk against the live ontology's own entity types in milliseconds. A chunk that
    names none of them clears no LLM call at all, `graph.build.extract_and_consolidate`'s own gate
    ahead of `extract.llm.combined_extract`. In-process only, no serving lane exists for it since
    it is too fast relative to an HTTP round trip to be worth one. A `patos` singleton, the
    checkpoint loaded once on first `EntityGate()` construction, so `ops.setup()` must have
    ...
    model: the loaded gliner2 checkpoint, `settings.gliner_gate_model` on `settings.
        gliner_gate_device`.
    labels: the schema every chunk is scored against, `ontology.gate_labels()`'s active entity
        kind names minus `Concept`. Concept is the extractor's own explicit catch-all ("when
        nothing fits use Concept"), and calibration against real prose showed it matches nearly
        any noun phrase (a plain "the weather" scored 0.79), which would make the gate pass
        everything. Dropping it widened the measured separation between filler and
        ontology-bearing text from roughly 0.5-0.8 down to 0.0-0.5 for filler and 0.9+ for the
        real thing.
    ...
    def __init__(self) -> None:
        ...
        self.labels = ontology.gate_labels()
