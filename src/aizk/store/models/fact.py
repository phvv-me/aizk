...
from ..mixins.scoped import OWNER_ID, SCOPE
from ..rls import Policy, content_policies, curation_admin_policies, default_scope_policies
...
class FactContent(Id, Embedded, TableBase, table=True):
    ...
    object_id: uuid.UUID | None = Field(
        default=None,
        sa_column=Column(ForeignKey("entity_content.id", ondelete="CASCADE"), index=True),
    )
    ...
    def __rls_policies__(cls) -> list[Policy]:
        ...
        return content_policies("fact_claim")
...
class FactClaim(Id, Scoped, TableBase, table=True):
    ...
    content_id: uuid.UUID = Field(
        sa_column=Column(
            ForeignKey("fact_content.id", ondelete="CASCADE"), nullable=False, index=True
        )
    )
    ...
    source_chunk_id: uuid.UUID | None = Field(
        default=None,
        sa_column=Column(ForeignKey("chunk.id", ondelete="SET NULL"), index=True),
    )
    promoted_from: uuid.UUID | None = Field(
        default=None,
        sa_column=Column(ForeignKey("fact_claim.id", ondelete="SET NULL"), index=True),
    )
    ...
    def __rls_policies__(cls) -> list[Policy]:
        ...
        `curation_admin_policies` rides alongside the ordinary scope policies as additive
        PERMISSIVE policies Postgres already ORs together per command.
        """
        return [
            *default_scope_policies(OWNER_ID, SCOPE),
            *curation_admin_policies(SCOPE),
        ]
    ...
    def __table_args__(cls) -> tuple[Index, ...]:
        ...
        # index since Postgres allows no WHERE clause on a table constraint, only on an index, and
        # NULLS NOT DISTINCT folds a private claim's null scope into the same uniqueness class as a
        # scoped one. No `*super().__table_args__` here: unlike `FactContent`, a claim mixes in no
        # `Embedded` and declares no embedding column to extend.
        return (
