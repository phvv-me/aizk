...
client connected and one note stored. None of it has to be done in order, but doing it in order is
the fastest way to find out whether aizk is worth keeping.

You talk to your assistant the whole time. The tool calls shown are what it sends on your behalf,
and you never type them yourself.

<Diagram
  title="Seven steps from your first brief to seeing your memory in the web app"
  nodes={[
    { id: 's1', label: 'store a brief', col: 0, row: 0 },
    { id: 's2', label: 'ask it back', col: 0, row: 1 },
    { id: 's3', label: 'add a document', col: 0, row: 2 },
    { id: 's4', label: 'check standing', col: 0, row: 3 },
    { id: 's5', label: 'write to a team', col: 0, row: 4 },
    { id: 's6', label: 'correct it', col: 0, row: 5 },
    { id: 's7', label: 'open the web app', col: 0, row: 6, emphasis: true },
  ]}
  edges={[
    { from: 's1', to: 's2' },
    { from: 's2', to: 's3' },
...
We validate the new binding assay before the summer run. We picked the plate reader over the
imaging rig because the imaging rig cannot hold temperature for a nine hour plate.""")
```

Two things are happening. The prose is what find will hand back later, and the tagged lines
...
second call. [Writing memory well](/docs/user/using/keep/) covers where the line is.
...
```python
aizk.find(query="why did we choose the plate reader for the assay?")
```
...
original, and converts it into something findable.
...
PDF takes a while, and the note becomes findable when conversion finishes rather than instantly.
...
:::caution[Read the flag, do not guess]
Being able to read a team's memory does not mean you can write to it, and organization names have
to match exactly what identity says they are.
:::
...
Ordinary find now favors the current version. The old one keeps its dates and stays in history,
...
and one correction. That is enough to tell whether find gives back something you would have had
...
- [Writing memory well](/docs/user/using/keep/) is the next thing to read.
