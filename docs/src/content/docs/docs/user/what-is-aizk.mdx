...
aizk is a memory you and your AI assistants share. You tell it things worth keeping, it keeps
them, and later any assistant you use can ask for what is relevant and get it back in the
original words, with a note about where it came from.
...
<Diagram
  title="You and a teammate each work through your own assistant, and both assistants share one aizk"
  nodes={[
    { id: 'you', label: 'you', col: 0, row: 0 },
    { id: 'ya', label: 'your assistant', col: 1, row: 0 },
    { id: 'tm', label: 'a teammate', col: 0, row: 2 },
    { id: 'ta', label: 'their assistant', col: 1, row: 2 },
    { id: 'm', label: 'aizk', col: 2, row: 1, emphasis: true },
  ]}
  edges={[
    { from: 'you', to: 'ya' },
    { from: 'ya', to: 'm', label: 'keep, find' },
    { from: 'tm', to: 'ta' },
    { from: 'ta', to: 'm', label: 'keep, find' },
  ]}
/>
...
## It keeps when things were true
...
AIZK has two supported deployment profiles. Both keep documents, claims, vectors, temporal state,
scopes and durable work in one SQL backend. Both expose the same five MCP tools.
...
The profiles make different operational tradeoffs. The CockroachDB profile uses C-SPANN and a
portable durable queue. The PostgreSQL profile uses VectorChord, BM25 and PgQueuer. Read
[Database profiles](/docs/dev/run/databases/) before choosing one.
...
- [Database profiles](/docs/dev/run/databases/) compares the supported ways to run AIZK.
