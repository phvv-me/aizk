...
[Quickstart](/docs/user/quickstart/) covers. It is about the shape of what you write rather than
the mechanics of writing it.

Writing to aizk is one call named `remember` holding self-describing Markdown. Everything else is
optional, and the defaults are almost always the right answer.
...
Write the reasoning, not only the outcome. A note saying the team uses one library is worth
little in six months. A note saying the team moved to that library because the previous one held a
lock across an await and stalled the queue is worth a lot, because it tells the next reader when
the decision stops applying.

Keep secrets out. Credentials, private keys, tokens, and anything you would not want in a database
backup do not belong in memory, and there is no mechanism to take them back out cleanly once they
are in.
...
they explain each other, they belong in one note, because splitting them means recall can return
one without the two that give it context.
...
The first level-one heading is what recall shows as the title, so make it the thing you would
search for rather than a label like Notes or Update.
...
Source tags attach a note to named things so related notes cluster. The general form is a kind and
a name, and the two common ones are project and area.
...
real date differs materially from today, such as recording a decision made three months ago. Set
the second only when the world supplies a genuine deadline, such as a contract that ends.

Expiry is a hard boundary rather than a reminder. Never use it to mean the note might go stale
eventually, because when it passes the note stops showing up in ordinary recall.
...
Naming no organization writes privately, and that is the default. Naming one writes into a team
scope, and your assistant can only name organizations you are actually allowed to write to.
[Sharing and organizations](/docs/user/using/sharing/) covers checking that before the first shared
write.

The habit worth forming is writing straight into the scope where the note belongs, rather than
writing privately and sharing later. Sharing later makes a copy, and copies drift.
