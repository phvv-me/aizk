...
[Writing memory well](/docs/user/using/keep/) covers. It is about the other kind of thing you
...
```text
aizk.keep(source_uri="https://example.org/binding-assay-protocol.pdf")
```
...
<Diagram
  title="A URL is fetched once, scanned, preserved, and converted into findable text"
  nodes={[
    ...
    { id: 'find', label: 'findable', col: 0, row: 5, emphasis: true },
...
on. The text is stored as companion context on the same revision, so find can return your framing
...
```sh
aizk keep report.pdf notes.md --text "Primary references for the current design."
```
...
That note is findable. So a file that failed to convert is still findable by name and still tells
...
sources that find then has to reassemble. Store a note describing what the file is and where it
lives instead.

## Getting the original bytes back

Find stays text first. When an evidence item is grounded in a stored file, it can name the exact
