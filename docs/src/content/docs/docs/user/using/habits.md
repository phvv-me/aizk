...
The opposite failure is real too. A single note covering four unrelated projects matches everything
weakly and helps with nothing. One topic, written whole, is the target.

## Text beats a preserved file

Store what you understood, not the thing you read. Your own paragraph is shorter, it is searchable at
once, and it says why the document mattered, which the document never does.

Preserve the original only when the exact bytes might be needed later. A contract, a signed record, a
form, a paper you might quote by page, and a slide deck somebody will reopen all qualify.
[Files, PDFs and web sources](/docs/user/using/files/) covers how, and its best advice is that a link
plus your own summary usually beats a stored copy.
...
Adding a contradicting note instead is the fastest way to make a memory untrustworthy, because after
it happens twice nobody can tell which of any two notes to believe.

## Write the why, not only the what

A note that records a decision is worth something. A note that records the reason is worth something a
year later, because the reason tells the next reader when the decision stops applying.
...
section ever drops below the cost of running our own, this is worth redoing.
```

That last sentence is what makes it a good note. Somebody reading it in a year knows exactly what
would change the answer, and does not have to rebuild the trade from scratch.

Be just as direct about what you do not know. A measurement taken once on one machine should say so,
and a comparison with no head-to-head number behind it should say that too. An honest gap is useful
and a confident guess is a trap.
...
:::caution[A tag is not a workflow board]
A tag saying urgent, in progress, or blocked is true for about a week and then quietly lies forever,
because nothing goes back to remove it. When state genuinely matters, write it in the prose where a
reader can see the date around it, or use an explicit typed relation.
:::
...
early. [Entities, facts, ontology](/docs/user/concepts/graph/) covers typed relations.
...
version control there, they are already searchable there, and a copy in memory starts drifting the day
after it is made.
