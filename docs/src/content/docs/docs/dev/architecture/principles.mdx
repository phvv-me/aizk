...
they are worth a read before your first contribution, because half of them are enforced by a gate
and the other half are conventions that reviewers hold you to. The useful part is knowing which is
which, so this page says so, checked against the code rather than repeated from the old
architecture note. It assumes you have read
[Layers and import contracts](/docs/dev/architecture/layers/).

## Agentic first

The primary caller is an agent, not a person. The MCP server is the interface that gets designed
first, and it exposes five tools, `status`, `find`, `keep`, `report` and `share`, plus one resource
...
The consequences run deep. `find` returns evidence rather than an answer, because an agent can
...
| vector search | C-SPANN on CockroachDB or VectorChord on PostgreSQL |
| job queuing | the portable CockroachDB queue or PgQueuer on PostgreSQL |
| model serving | an OpenAI-compatible hosted endpoint or vLLM |
| chunking | `chonkie` |
| the MCP protocol | FastMCP |
| row level security machinery | the house `rls` package |

The PostgreSQL profile includes one model server in `src/services/gliner/app.py` because GLiNER has
no vLLM-compatible image. The CockroachDB profile adds a small queue adapter because PgQueuer uses
PostgreSQL behavior CockroachDB does not provide. [Database profiles](/docs/dev/run/databases/)
owns the exact boundary.

## The eight rules and what actually enforces them

<Diagram
  title="Where a new piece of code belongs, decided by three questions"
  nodes={[
    { id:'new', label:'new code', col:1, row:0 },
    { id:'q1', label:'composes SQL?', col:1, row:1 },
    { id:'sql', label:'store, or the twelve', col:0, row:1 },
    { id:'q2', label:'builds a service?', col:1, row:2 },
    { id:'rt', label:'Runtime.assemble', col:0, row:2 },
    { id:'q3', label:'emits user markdown?', col:1, row:3 },
    { id:'tpl', label:'a Jinja template', col:0, row:3 },
    { id:'beside', label:'beside its code', col:1, row:4, emphasis:true },
  ]}
  edges={[
    { from:'new', to:'q1' },
    { from:'q1', to:'sql', label:'yes' },
    { from:'q1', to:'q2', label:'no' },
    { from:'q2', to:'rt', label:'yes' },
    { from:'q2', to:'q3', label:'no' },
    { from:'q3', to:'tpl', label:'yes' },
    { from:'q3', to:'beside', label:'no' },
  ]}
/>
...
templates, `src/aizk/retrieval/templates/find.md.j2` for the find answer and
...
rest survive only because somebody reads the diff.

:::tip[Found a convention being broken?]
Treat it as a bug in the tree, not a stale rule. The honest fix is either to repair the code or to
write the gate that would have caught it.
:::
