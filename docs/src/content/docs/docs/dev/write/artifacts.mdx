...
only theoretical. Raise the three ClamAV variables together for more, and keep the whole
...
`ArtifactProcessor` reads the original back and posts it to Docling Serve asking for `md` alone with
...
metadata-only shell is findable but never becomes facts.
