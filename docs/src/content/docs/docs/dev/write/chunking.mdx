...
`markdown`, `json` and `toml`. The ingest path does not call `is_code`, so every source is chunked
as prose.
...
carrying the speaker label and role with `observed_at` and `expires_at`, and find reads the
...
[re-chunk sweep](/docs/dev/interfaces/cli/) fixes.
...
Three things drain that backlog. `Memory.keep` calls `enqueue_document` right after ingest, so a
