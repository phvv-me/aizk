...
read [how find runs](/docs/dev/read/overview/) and know that step four builds one cached
...
| `VectorLane` | `working_memory` | unpromoted `session_item` rows | `session_find_k` |
| `VectorLane` | `profile` | entity profile summaries | `profile_find_k` |
| `VectorLane` | `communities` | community label and summary lines | `community_find_k` |
