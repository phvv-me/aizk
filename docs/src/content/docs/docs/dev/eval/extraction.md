...
The extraction contract is checked against a committed public fixture covering architecture,
authentication, hashing, artifacts, and the MCP interface. Earlier variants failed for interface
reasons rather than model quality. A nullable wire quote let the model omit evidence, legal JSON
whitespace exhausted a bounded response, and the phrase shortest quote invited ellipses across
separate source spans. Requiring a quote, compact XGrammar output, and one contiguous
character-for-character substring fixed all three.
...
Two smoke cells used representative public research chunks. In the CPU comparison, GLiNER took
10.9 to 15.5 seconds while the LLM took 20.1 to 60.1 seconds on a GPU. The LLM produced 31 facts
across four chunks. GLiNER base produced eight and emitted nothing on two. Exact triple agreement
was zero, and several GLiNER relations did not express the source meaning.
...
A manual reading of eight cross-document questions found three strong answers, two partial answers,
and three justified abstentions. The result checks the workflow, but it is not a general quality
claim.
...
| Cerebras through OpenRouter | pinned with `provider.only` and fallbacks disabled | provider claims are only planning inputs, so measure throughput and cost on the active account before a large run |
...
:::caution
Everything on this page is a small diagnostic on controlled fixtures, not a published quality benchmark. No
