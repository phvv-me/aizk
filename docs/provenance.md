...
| FastMCP server with Zitadel JWT/introspection and local-key token verifiers | FastMCP (library), Zitadel (identity provider) | `mcp/server.py` (`AizkMCP`), `store/models/principal.py` (`Principal.verifier`, `Principal.from_token`) | shipped |
