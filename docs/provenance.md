...
| Active-record model methods: each ORM class carries its own read and write operations (`Group.create`, `Group.pending_facts`, `Principal.for_subject`, `FactClaim.archive_stale`, `Watermark.bump`) rather than a separate repository or service layer, so a caller reasons about one object's own API instead of a parallel query module per table | standard active-record pattern (Rails/Django ORM lineage), applied atop SQLModel | `store/models/*.py` | shipped |

## Extraction and the graph tiers

| Feature | Source | Where in code | Status |
|---|---|---|---|
| Ontology-driven, closed-vocabulary extraction | TRACE-KG, arXiv 2604.03496 | `extract/ontology.py` (`EntityType`, `RelationType`, `ONTOLOGY_PROMPT`), `extract/strategies.py` | shipped |
...
| Anonymous read access with a token-bucket rate limit, write refused | original (aizk) | `mcp/middleware.py` (`AnonymousRateLimit`), `mcp/principal.py` (`require_identified`) | shipped |
| The narrowing lens, projecting one group's composed graph out of the visible union | original (aizk) | `store/context.py` (`acting_as`, `scope` argument), `store/rls/predicates.py` (`LENS`) | shipped |
...
| FastMCP server with Zitadel JWT/introspection and local-key token verifiers | FastMCP (library), Zitadel (identity provider) | `mcp/server.py` (`AizkMCP`), `auth/tokens.py` | shipped |
