...
runner=$(realpath hackathon/video/full-agent-demo.sh)
...
Xvfb "$display_name" -screen 0 1920x1080x24 -nolisten tcp > /tmp/craizk-full-agent-xvfb.log 2>&1 &
display_pid=$!
sleep 1
...
DISPLAY="$display_name" ffmpeg -hide_banner -loglevel error -y \
  ...
  -t 900 -an -c:v libx264 -preset veryfast -crf 18 -pix_fmt yuv420p \
  -movflags +faststart "$output_root/05-live-agent-workflow.mp4" &
recorder_pid=$!

DISPLAY="$display_name" dbus-run-session -- gnome-terminal --wait --hide-menubar \
  --geometry=145x39+0+0 --zoom=1.30 -- "$runner" > /tmp/craizk-full-agent.log 2>&1 &
terminal_pid=$!
...
test -s "$output_root/05-live-agent-workflow.mp4"
ffprobe -v error -select_streams v:0 -show_entries stream=width,height,r_frame_rate \
  -show_entries format=duration -of default=noprint_wrappers=1 \
  "$output_root/05-live-agent-workflow.mp4"
