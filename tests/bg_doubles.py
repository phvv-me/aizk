...
def patch_queue_seam(monkeypatch: pytest.MonkeyPatch, module: object) -> RecordingQueue:
    ...
    Every connection opens inside `queue.queue_connection`, so the connect fake always lands on
...
class RecordingPg:
    ...
    `run`; this double keeps the decorated bodies so a test drives them directly and records
    the one `run` call so the worker's start contract is checked without a database.
    ...
    def entrypoint(
        self, name: str, concurrency_limit: int | None = None
    ) -> Callable[[Callable[..., Awaitable[None]]], Callable[..., Awaitable[None]]]:
        """Record and return a queue entrypoint registration decorator.

        name: entrypoint the decorated body drains.
        concurrency_limit: per-entrypoint task cap the worker sets (extraction), recorded here so a
            test can assert it without pgqueuer; accepted and ignored beyond that.
        """
    ...
    async def run(self, batch_size: int = 10, max_concurrent_tasks: int | None = None) -> None:
        ...
        max_concurrent_tasks: process-wide task ceiling the worker sets, recorded and ignored.
        """
