...
def fact_result(query: str, statement: str) -> RecallResult:
    """A recall bundle carrying one fact, the surface the sweep scores a hit against.
...
class FakeMeter:
    ...
    the one external boundary the offline sweep tests replace.
    """
...
def patch_recall_and_meter(monkeypatch: pytest.MonkeyPatch, surfaced: str) -> None:
    """Stub recall to a fixed fact and the meter to fixed peaks, the offline sweep harness.

    monkeypatch: the fixture the stubs install through.
    surfaced: the statement every stubbed recall returns, the gold the run scores a hit on.
    """
    ...
    async def stub_recall(
        ...
        return fact_result(query, surfaced)

    monkeypatch.setattr(sweep_module, "recall", stub_recall)
...
def sweep_matrices() -> st.SearchStrategy[SweepMatrix]:
    ...
    Each axis draws a small unique list, some empty, so the property covers both the cartesian
    product and the empty-axis-holds-the-live-setting fallback in one go, bounded so the grid stays
    small enough to expand in a property.
    """
    return st.builds(
        SweepMatrix,
...
def test_config_result_reads_metrics_latency_and_halfvec_footprint(
    ...
    assert row.storage_bytes_per_vector == width * 2  # a halfvec element is two bytes


@pytest.mark.parametrize("mode", ["compare", "single", "synth", "empty"], ids=lambda mode: mode)
def test_run_sweep_scores_each_config_or_short_circuits_offline(
    ...
    table. synth: a null gold is replaced by questions synthesized from the corpus, a self-probe.
    empty: an empty gold short-circuits to an empty scorecard before any config is scored.
    """
    if mode != "empty":
        patch_recall_and_meter(monkeypatch, "alpha holds")
    ...
    if mode == "synth":
        ...
        monkeypatch.setattr(sweep_module, "build_questions", stub_build_questions)
        report = asyncio.run(run_sweep(questions=None, k=4, matrix=matrix))
    elif mode == "empty":
        report = asyncio.run(run_sweep(k=4, gold=[]))
    else:
        gold = [QA(question="what does alpha hold", expected="alpha holds")]
        report = asyncio.run(run_sweep(k=4, matrix=matrix, gold=gold))
    ...
    for row in report.results:
        ...
        assert row.peak_host_gb == 1.5 and row.peak_gpu_gb == 0.5
        assert row.storage_bytes_per_vector == settings.embed_dim * 2
            ...
            SweepReport(
                ...
                results=[
                    ConfigResult(
                        ...
                        peak_host_gb=1.5,
                        peak_gpu_gb=0.5,
                        storage_bytes_per_vector=2048,
                    )
                ],
                comparison="table",
                best_label=None,
            ),
            ["recall@4=", "p50=", "p95=", "store=2048b", "rerank="],
        ),
        (
            SweepReport(n=0, k=4, results=[], comparison=None, best_label=None),
            ["no configs"],
        ),
    ],
)
def test_render_renders_a_row_per_config(report: SweepReport, needles: list[str]) -> None:
    """The rendered table carries the metrics, latency, memory, and storage of each config."""
