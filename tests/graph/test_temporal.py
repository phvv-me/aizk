...
def test_record_access_bumps_recency_and_count_and_no_ops_on_empty() -> None:
    async def body() -> tuple[int, bool]:
        ...
        async with dbutil.actor(owner) as session:
            await Fact.Claim.record_access(session, [])
            await Fact.Claim.record_access(session, [claim])
        async with dbutil.actor(owner) as session:
            row = (await session.exec(select(Fact.Live).where(Fact.Live.id == claim))).first()
