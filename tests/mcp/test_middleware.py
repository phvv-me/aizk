...
class FakeFastmcpContext:
    ...
    state: the initial Context state, the resolved `Principal` a downstream middleware reads back.
    """
...
def test_principal_middleware_resolves_once_and_stashes_it_for_the_call(
    ...
    result = dbutil.run(PrincipalMiddleware().on_call_tool(context, call_next))
    assert result == "ok"
...
def test_rate_limit_lets_any_non_anonymous_call_pass_uncharged(state: object) -> None:
    ...
    seed = {PRINCIPAL_STATE_KEY: state} if state is not None else {}
...
async def _async_return[T](value: T) -> T:
    """Await to `value`, the coroutine a patched zero-arg `resolve_principal` returns."""
    return value
